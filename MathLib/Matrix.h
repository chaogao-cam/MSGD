 ...
 * This should be combined with the Vector class for doing almost anything
...
public:
    ...
    /// Add a row inited with zeros above the first row
    inline virtual Matrix& AddRow()
    {
        REALTYPE *arr = new REALTYPE[(row+1)*column];
        
        //fill first row by 0
        memset(arr,0,column*sizeof(REALTYPE));
        
        //copy the original matrix from second row
        memcpy(arr+column,_,row*column*sizeof(REALTYPE));
