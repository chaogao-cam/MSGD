...
public:
    ...
    // for DTW
 inline Vector&
    ReverseAndResizeByActualLength
    (unsigned int actualLength)
    {
        Resize(actualLength,true);

        REALTYPE tmp;
        
        for(unsigned int i=0; i < row-1-i; i++)
        {
            tmp = _[i];
            _[i] = _[row-1-i];
            _[row-1-i] = tmp;
    ...
    /// Get the size of unique elements in the vector
    inline unsigned int SizeOfUniqueElements() const{
